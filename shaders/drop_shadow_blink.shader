...
float4 mainImage(VertData v_in) : TARGET
{
   ...
   float4 final_shadow_color = float4(shadow_color.rgb, shadow_color.a * sampled_shadow_alpha* (1 + sin(t)) / 2);
   float4 color_two = final_shadow_color * (1-color.a) + color * (int(is_alpha_premultiplied) * 1 + int(!is_alpha_premultiplied) * color.a);
   return float4(color_two.r, color_two.g, color_two.b, color_two.a);
}
